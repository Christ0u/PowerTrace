...
class INA228:
    ...
    def shunt_tempco(self):
        self.write_register16(
            self.__INA228_SHUNT_TEMPCO,
            INA228_SHUNT_TEMPCO_VALUE)
    ...
    def get_charge(self):
        ...
        # charge = raw * self.get_current_lsb()
        # return charge
        raw = self.read_register40(self.__INA228_CHARGE)
        if raw & (1 << 39):
            raw -= (1 << 40)
        return raw * self.get_current_lsb()
    ...
    def set_shunt_overvoltage(self, value):
        if value >= 0:
            ...
            value_temp = value * (-1)
            data = (value_temp * INA228_SHUNT_OHMS) / \
                self.get_shunt_conv_factor()
            data = ~data
            data = data + 1
